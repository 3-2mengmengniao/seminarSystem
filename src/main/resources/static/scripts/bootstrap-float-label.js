    ...
    var FloatingLabel = function (element, options) {
        ...
        this.$input.on('focus',function () {
            ...
            if(typeof that.$label.animate === 'function'){
                that.$label.animate({
                    top:'-' + (that.positionData.top + that.positionData.top-10) + 'px',
                    left: 0,
                },that.options.animationDuration);
                that.$label.css('font-size','14px');
            }
            else {
                that.$label.css('top', '-' + (that.positionData.top + that.positionData.top-10) + 'px');
